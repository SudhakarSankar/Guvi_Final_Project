...
# streamlit run Guvi_Final_Project.py
...
with st.sidebar:
    select = option_menu(
        "MAIN MENU", ["Home", "Predict Conversion", "About"],
...
if select == "Home":
    # img = Image.open(r"C:\Path\To\Ecommerce_Image.jpg")  # Update with your image path
    # st.image(img, width=600)

    st.markdown("<h2 style='color:green;'>E-commerce Visitor Behavior:</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 style='color:green;'>Visitor Tracking:</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 style='color:green;'>Conversion Factors:</h2>", unsafe_allow_html=True)

    st.write('''Several factors influence visitor conversion, including product pricing, user experience, targeted ads,
    and personalized recommendations.''')

    st.markdown("<h2 style='color:green;'>Machine Learning in Conversion Prediction:</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 style='color:green;'>Business Impact:</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h2 style='color:green;'>A/B Testing and Optimization:</h2>", unsafe_allow_html=True)

    st.write("By testing different strategies, businesses can refine their approach to maximize conversion rates.")

    st.markdown("<h2 style='color:green;'>Real-time Prediction:</h2>", unsafe_allow_html=True)
    ...
    with col1:
        count_session = st.number_input("Session Count (1 - 143)", min_value=1, max_value=143, value=(1+143)//2, step=1)
        totals_newVisits = st.number_input("New Visits (0 - 1)", min_value=0, max_value=1, value=0, step=1)
        device_operatingSystem = create_dropdown("Operating System", MAPPINGS["DEVICE_OPERATING_SYSTEM"], col1)
        device_deviceCategory = create_dropdown("Device Category", MAPPINGS["DEVICE_DEVICE_CATEGORY"], col1)
        historic_session_page = st.number_input("Historic Sessions (1 - 3066)", min_value=1, max_value=3066, value=(1+3066)//2, step=10)
        avg_session_time_page = st.number_input("Avg Time per Page (sec) (1 - 3698)", min_value=1, max_value=3698, value=(1+3698)//2, step=10)
        sessionQualityDim = st.number_input("Session Quality Score (1 - 67)", min_value=1, max_value=67, value=(1+67)//2, step=1)
        earliest_visit_number = st.number_input("Earliest Visit Number (1 - 528)", min_value=1, max_value=528, value=(1+528)//2, step=1)
        visits_per_day = st.number_input("Visits Per Day (1 - 7758)", min_value=1, max_value=7758, value=(1+7758)//2, step=10)
        earliest_medium = create_dropdown("Earliest Medium", MAPPINGS["EARLIEST_MEDIUM"], col2)
        latest_keyword = st.number_input("Latest Keyword ID (1 - 572)", min_value=1, max_value=572, value=(1+572)//2, step=1)
        latest_isTrueDirect = create_dropdown("Latest True Direct?", MAPPINGS["LATEST_ISTRUEDIRECT"], col2)
        time_on_site = st.number_input("Time on Site (sec) (1 - 6002)", min_value=1, max_value=6002, value=(1+6002)//2, step=10)
        products_array = st.number_input("Products Viewed (1 - 3559)", min_value=1, max_value=3559, value=(1+3559)//2, step=1)

    with col2:
        count_hit = st.number_input("Total Hits (1 - 4474)", min_value=1, max_value=4474, value=(1+4474)//2, step=10)
        device_browser = create_dropdown("Device Browser", MAPPINGS["DEVICE_BROWSER"], col1)
        device_isMobile = create_dropdown("Is Mobile?", MAPPINGS["DEVICE_IS_MOBILE"], col1)
        geoNetwork_region = st.number_input("Region ID (1 - 91)", min_value=1, max_value=91, value=(1+91)//2, step=1)
        avg_session_time = st.number_input("Avg Session Time (sec) (1 - 8068)", min_value=1, max_value=8068, value=(1+8068)//2, step=10)
        single_page_rate = st.number_input("Single Page Rate (1 - 344)", min_value=1, max_value=344, value=(1+344)//2, step=1)
        earliest_visit_id = st.number_input("Earliest Visit ID (1 - 7639)", min_value=1, max_value=7639, value=(1+7639)//2, step=100)
        days_since_first_visit = st.number_input("Days Since First Visit (1 - 31)", min_value=1, max_value=31, value=(1+31)//2, step=1)
        earliest_source = create_dropdown("Earliest Source", MAPPINGS["EARLIEST_SOURCE"], col2)
        earliest_keyword = st.number_input("Earliest Keyword ID (1 - 410)", min_value=1, max_value=410, value=(1+410)//2, step=1)
        earliest_isTrueDirect = create_dropdown("Earliest True Direct?", MAPPINGS["EARLIEST_ISTRUEDIRECT"], col2)
        num_interactions = st.number_input("Total Interactions (1 - 6851)", min_value=1, max_value=6851, value=(1+6851)//2, step=10)
        transactionRevenue = st.number_input("Transaction Revenue (1 - 4755)", min_value=1, max_value=4755, value=(1+4755)//2, step=10)
    ...
    if st.button("Predict Conversion"):
        ...
        st.write(f"- **Device**:  {'Mobile' if device_deviceCategory == 1 else 'Tablet' if device_deviceCategory == 2 else 'Desktop'}")


        # Predict using trained model
        prediction = XGBoost_model.predict(user_data)
        # Display Prediction Result
        st.success(f"### Prediction: {'🟢 Converted' if prediction >= 0.5 else '🔴 Not Converted'}")
